"""Benchmark LayerNorm: eager PyTorch vs. torch.compile vs. fused Triton.

Memory-bound, so the headline metric is effective bandwidth (GB/s). Results
print as a table and save as a PNG for the README.

Usage:
    python benchmarks/bench_layernorm.py --M 8192 --dtype fp16
...
def bytes_moved(M, N, dtype):
    ...
    # read x + write y (M*N each); w, b negligible
    return (2 * M * N + 2 * N) * elem
...
def run(M, hidden_sizes, dtype, provider):
    ...
    for N in hidden_sizes:
        ...
        if provider == "eager":
            fn = lambda: layernorm_reference(x, w, b)
        elif provider == "compile":
            compiled = torch.compile(layernorm_reference)
            compiled(x, w, b)  # warm up outside timing
            fn = lambda: compiled(x, w, b)
        elif provider == "triton":
            fn = lambda: layernorm(x, w, b)
        else:
...
def main():
    ...
    p.add_argument("--out", type=str, default="assets/layernorm_bench.png")
    args = p.parse_args()
    ...
    print(f"\nLayerNorm  |  M={args.M}  dtype={args.dtype}  GPU={gpu}\n")
    ...
    _plot(data, hidden_sizes, args, dtype)


def _plot(data, hidden_sizes, args, dtype):
    try:
        import matplotlib
    ...
    ax1.set_title(f"LayerNorm latency (M={args.M}, {args.dtype})")
